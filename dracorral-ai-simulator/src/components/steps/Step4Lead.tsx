...
interface Step4LeadProps {
  ...
  isLoading: boolean;
  error: string | null;
  onSaveLead: (data: { nombre: string; email: string; telefono: string; onComplete: (code: string) => void }) => void;
  onBack: () => void;
}
...
export function Step4Lead({
  ...
  // Use either the global leadCode or the local one for the success view
  const code = leadCode || localCode;
    ...
    return (
      <main className="flex-1 flex flex-col items-center px-4 py-8 max-w-md mx-auto w-full">
        ...
        <div className="scale-in w-full space-y-5">
          ...
          {/* TODO Phase 7: Send confirmation email via Resend API
              The backend already saves the lead with status 'pending'.
              Email will be triggered by a Firestore onCreate Cloud Function.
              Template: welcome email with code + next steps. */}
    ...
    <main className="flex-1 flex flex-col px-4 py-6 sm:py-8 max-w-md mx-auto w-full">
      ...
      <div className="mb-5">
        ...
        <p className="font-sans text-sm text-muted leading-relaxed">
          Agenda tu sesión y recibe un código exclusivo de descuento.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4" noValidate>
        ...
        <p className="font-sans text-xs text-muted leading-relaxed">
          🔒 Tus datos solo se usan para contactarte sobre tu diagnóstico. No spam.
        </p>
